...
```
specs/service/<domain>/*.ssac  →  ssac parse  →  ssac validate  →  ssac gen  →  artifacts/<domain>/*.go
...
- 서비스 파일은 `.ssac` 확장자를 사용 (Go 컴파일러 빌드 충돌 방지)
- 하나의 `.ssac` 파일에 여러 함수 가능 (각 함수 위에 `// @` 시퀀스 주석)
...
서비스 파일은 반드시 도메인 서브 폴더에 위치해야 한다. `service/` 직접에 `.go` 파일을 두면 ERROR가 발생한다.

```
specs/service/
├── auth/
│   └── login.go              ← Domain="auth", package auth
├── reservation/
│   └── cancel_reservation.go ← Domain="reservation", package reservation
└── room/
    └── update_room.go        ← Domain="room", package room
```

`service/*.go` (flat 구조)는 금지된다.
...
- SSaC 파라미터명 ↔ interface 파라미터명 매칭 검증 (`context.Context` 제외). 불일치/누락 → ERROR
- DDL 테이블 검증을 건너뜀
...
```
<project-root>/
  service/<domain>/  # sequence 주석 파일 (*.ssac, 도메인 서브 폴더 필수)
...
```go
// specs/service/reservation/cancel_reservation.go
package reservation
...
```go
// artifacts/reservation/cancel_reservation.go
package reservation
